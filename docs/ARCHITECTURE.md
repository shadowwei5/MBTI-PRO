...
> 版本: v2.1 | 日期: 2026-06-19 | 状态: MVP完成，生产部署配置就绪(vercel.json+render.yaml+start.sh)
...
| 问题 | 状态 | 原因 |
|------|------|------|
| ~~EBCJ/ENFJ/INFJ 角色部位被误删~~ | ✅ 已解决 | 人工审核图片替换 |
| ~~火山引擎 API 不可用~~ | ✅ 无需调用 | 人工审核图片已覆盖 72/81 类型 |
| 琥珀组 9 型皮肤斑点 | ⚠️ 待修复 | S×F 类型皮肤区域经肤色保护后仍有纹理瑕疵，人工审核未通过。当前保留 `adjust_colors_v3.py` 处理版本。不影响 UI 正常展示。 |
